...
dataset="tugraz" #Dataset you are using
...
num_val_images=20 #The number of images to used for validations
...
brightness=True #Whether to randomly change the image brightness for data augmentation. Specifies the max bightness change as a factor between 0.0 and 1.0. For example, 0.1 represents a max brightness change of 10%% (+-).
rotation=True #Whether to randomly rotate the image for data augmentation. Specifies the max rotation angle in degrees.
model= "FRRN-A"
...
network, init_fn = model_builder.build_model(model_name=model, net_input=net_input, num_classes=num_classes, crop_width=crop_width, crop_height=crop_height, is_training=True)
...
for epoch in range(epoch_start_i, num_epochs):
    ...
    if epoch % validation_step == 0:
        ...
        for ind in val_indices:
            ...
            input_image = np.expand_dims(np.float32(utils.load_image(val_input_names[ind])[:crop_height, :crop_width]),axis=0)/255.0
            gt = utils.load_image(val_output_names[ind])[:crop_height, :crop_width]
    ...
    plt.savefig('accuracy_vs_epochs'+str(epoch)+'.png')
    ...
    plt.savefig('loss_vs_epochs'+str(epoch)+'.png')
    ...
    plt.savefig('iou_vs_epochs'+str(epoch)+'.png')
